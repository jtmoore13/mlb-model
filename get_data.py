...
def add_yesterday_off(box_score, season_games):
    ...
    """
    date = utils_data.str_to_datetime(box_score['date'])
    day_before = utils_data.get_day_before(date)
...
def get_bullpen_stats(season_games, pitcher_data, team_abbr, year):
    ...
    """
    print(f'{utils_data.BACKSPACE*6} {team_abbr} {utils_data.CHECK}', end='', flush=True)
    ...
    team_bullpen = {}
    for game in season:
        if utils_data.game_suspended(game):
            continue
        date = utils_data.get_stat_value(game, 'date_game')
        if date in team_bullpen:
            date += ' (2)'
        opp_abbr = utils_data.get_stat_value(game, 'opp_ID')
        ...
        if not utils_data.starter_verified(last_name, full_name):
            continue

        starter_stats = pitcher_data[starter_id][date]
        game_H = utils_data.get_stat_value(game, 'H') - starter_stats['H']
        game_IP = utils_data.add_IP(utils_data.get_stat_value(game, 'IP'), -starter_stats['IP'])
        game_BB = utils_data.get_stat_value(game, 'BB') - starter_stats['BB']
        game_ER = utils_data.get_stat_value(game, 'ER') - starter_stats['ER']
        ...
        if season_IP > 0:
            team_bullpen[date]['pregame_ERA'] = utils_data.calculate_ERA(season_ER, season_IP)
            team_bullpen[date]['pregame_WHIP'] = utils_data.calculate_WHIP(season_IP, season_BB, season_H)

        season_H += game_H
        season_IP = utils_data.add_IP(season_IP, game_IP)
        season_BB += game_BB
        season_ER += game_ER
...
def calculate_pitcher_stats(pitcher_data):
    ...
    for pitcher in pitcher_data:
        ...
        for date in appearences:
            ...
            if season_IP > 0:
                game['pregame_ERA'] = utils_data.calculate_ERA(season_ER, season_IP)
                game['pregame_WHIP'] = utils_data.calculate_WHIP(season_IP, season_BB, season_H)
            season_ER += game['ER']
            season_BB += game['BB']
            season_H += game['H']
            season_IP = utils_data.add_IP(season_IP, game['IP'])
    print(utils_data.DONE)
...
def calculate_offensive_stats(season_games):
    ...
    for team in season_games:
        ...
        for i in range(1, len(dates)):
            ...
            if home_AB > 0:
                game['home_BA'] = utils_data.calculate_BA(home_AB, home_H)
                game['home_OBP'] = utils_data.calculate_OBP(home_H, home_BB, home_HBP, home_AB, home_SF)
                game['home_SLG'] = utils_data.calculate_SLG(home_1B, home_2B, home_3B, home_HR, home_AB)
                game['home_OPS'] = utils_data.calculate_OPS(game['home_OBP'], game['home_SLG'])
            if away_AB > 0:
                game['away_BA'] = utils_data.calculate_BA(away_AB, away_H)
                game['away_OBP'] = utils_data.calculate_OBP(away_H, away_BB, away_HBP, away_AB, away_SF)
                game['away_SLG'] = utils_data.calculate_SLG(away_1B, away_2B, away_3B, away_HR, away_AB)
                game['away_OPS'] = utils_data.calculate_OPS(game['away_OBP'], game['away_SLG'])
            if right_AB > 0:
                game['right_BA'] = utils_data.calculate_BA(right_AB, right_H)
                game['right_OBP'] = utils_data.calculate_OBP(right_H, right_BB, right_HBP, right_AB, right_SF)
                game['right_SLG'] = utils_data.calculate_SLG(right_1B, right_2B, right_3B, right_HR, right_AB)
                game['right_OPS'] = utils_data.calculate_OPS(game['right_OBP'], game['right_SLG'])
            if left_AB > 0:
                game['left_BA'] = utils_data.calculate_BA(left_AB, left_H)
                game['left_OBP'] = utils_data.calculate_OBP(left_H, left_BB, left_HBP, left_AB, left_SF)
                game['left_SLG'] = utils_data.calculate_SLG(left_1B, left_2B, left_3B, left_HR, left_AB)
                game['left_OPS'] = utils_data.calculate_OPS(game['left_OBP'], game['left_SLG'])
            ...
            for n in [10, 15]:
                ...
                game[f'{n}-day_BA'] = utils_data.calculate_BA(recent_AB, recent_H)
                game[f'{n}-day_OBP'] = utils_data.calculate_OBP(recent_H, recent_BB, recent_HBP, recent_AB, recent_SF)
                game[f'{n}-day_SLG'] = utils_data.calculate_SLG(recent_1B, recent_2B, recent_3B, recent_HR, recent_AB)
                game[f'{n}-day-OPS'] = utils_data.calculate_OPS(game[f'{n}-day_OBP'], game[f'{n}-day_SLG'])
...
def get_season_pitching(year, season_games):
    ...
    for game in schedule:
        ...
        if utils_data.has_not_happened(game_id):
            break
        game_page = SESSION.get('https://www.baseball-reference.com/' + game_id).text
        soup = BeautifulSoup(game_page, 'lxml')
        if utils_data.is_playoffs(soup):
            break
        if utils_data.was_suspended(soup):
            continue

        date, away_abbr, home_abbr = utils_data.parse_title(soup.title.text)
        if utils_data.is_second_game(soup):
            date += ' (2)'

        away_starter, home_starter = utils_data.get_starting_pitchers(soup)
        ...
        night_game = utils_data.get_night_game(soup)
        temp = utils_data.get_temperature(soup)
        ...
        for line in game_page.split('\n'):
            if team_totals == 2:
                break
            if not utils_data.is_pitching_row(line):
                continue
            if utils_data.has_team_totals(line):
                team = home_abbr
                opp = away_abbr
                team_totals += 1
                continue
        ...
        utils_data.print_same_line(f'{date} - {away_abbr} @ {home_abbr} {utils_data.CHECK}')
...
def get_season_offense(team_abbr, year):
    ...
    season_games = {}
    for row in table:
        if utils_data.game_suspended(row):
            continue
        box_score = {}
        for stat in ALL_STATS:
            box_score[stat] = utils_data.get_stat_value(row, stat)
        ...
        count += 1
    utils_data.print_same_line(f'{team_abbr} ({count} games) {utils_data.CHECK}')
...
def get_data(years):
    ...
    for year in years:
        ...
        teams = utils_data.get_team_abbreviations(year)
        ...
        print(utils_data.BACKSPACE*5+utils_data.DONE)
        ...
        utils_data.dump_data(year, 'team-bullpen-data.json', bullpen_data)
        utils_data.dump_data(year, 'pitcher-data.json', pitcher_data)
        utils_data.dump_data(year, 'game-data.json', season_games)
...
def main():
    ...
    years = [str(year) for year in range(model.START_YEAR, model.END_YEAR+1) if year != 2020]
    update = False
    ...
    if update:
        yesterday = utils_data.get_day_before(TODAY)
        latest = utils_data.format_date_long(yesterday)
